...
const Sales = ({ sales, setSales, products, customers, getStock, cashData, setCashData, currentUser, payments, returns = [] }) => {
  ...
  const livePrevBalance = useMemo(() => {
    ...
    // 5. Calculate Total Returns Adjustment from Returns history
    const totalReturned = (returns || [])
      .filter((r) => {
        const rCust = (r.customer || r.customerName || '').trim().toLowerCase();
        const matchName = rCust === normActiveName;
    ...
    // Total Previous Balance = Opening Balance + Credit Sales - Total Paid - Returns
    const calculatedPrev = Math.round(openingBal + totalSales - totalPaid - totalReturned);
    return Math.max(0, calculatedPrev);
  }, [sales, payments, returns, customers, activeCustomerName, customer]);
